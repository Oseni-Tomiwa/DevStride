...
function focusAreaAccessibleName(value: string, index: number): string {
  const normalized = value.trim().replace(/\s+/g, " ").slice(0, 80);
  return normalized ? `${normalized} focus area` : `focus area ${index + 1}`;
}
...
function DraftEditor({ items, setItems }: { items: FocusAreaDraft[]; setItems: (items: FocusAreaDraft[]) => void }) {
  return <div className="goal-focus-editor">
    {items.map((item, index) => <article className="goal-focus-draft" key={`${item.title}-${index}`}>
      ...
      <div className="goal-reorder-actions"><button type="button" className="button-quiet" onClick={() => index > 0 && setItems(items.map((value, itemIndex) => itemIndex === index - 1 ? items[index] : itemIndex === index ? items[index - 1] : value))} disabled={index === 0} aria-label={`Move ${focusAreaAccessibleName(item.title, index)} up`}>Move up</button><button type="button" className="button-quiet" onClick={() => index < items.length - 1 && setItems(items.map((value, itemIndex) => itemIndex === index + 1 ? items[index] : itemIndex === index ? items[index + 1] : value))} disabled={index === items.length - 1} aria-label={`Move ${focusAreaAccessibleName(item.title, index)} down`}>Move down</button></div>
    </article>)}
...
function ActiveGoal({ goal, initialProgress, onChanged, onArchived }: { goal: Goal; initialProgress: GoalProgress | null; onChanged: (goal: Goal) => void; onArchived: (goalId: string) => Promise<void> }) {
  ...
  const [editingFocus, setEditingFocus] = useState<Goal["focus_areas"][number] | null>(null);
  const [focusTitle, setFocusTitle] = useState("");
  const [archivingFocus, setArchivingFocus] = useState<Goal["focus_areas"][number] | null>(null);
  ...
  const archive = async () => { setBusy("archive"); setError(null); try { await archiveGoal(supabase, goal.id); setArchiveOpen(false); await onArchived(goal.id); } catch (cause) { setError(safeError(cause)); } finally { setBusy(null); } };
  ...
  const editFocusTitle = async () => { if (!editingFocus) return; const nextTitle = focusTitle.trim(); if (!nextTitle) { setError("A focus area title is required."); return; } if (nextTitle !== editingFocus.title) await updateFocus(editingFocus.id, { title: nextTitle }); setEditingFocus(null); };
  ...
  return <div className="goal-detail"><section className="goal-hero"><div><p className="eyebrow">Active goal · {goalTypeLabels[goal.goal_type]}</p>{editing ? <div className="goal-form"><label>Title<input value={title} onChange={(event) => setTitle(event.target.value)} /></label><label>Description<textarea value={description} onChange={(event) => setDescription(event.target.value)} /></label></div> : <><h2>{goal.title}</h2><p className="muted">{goal.description || "A focused plan for your next stage of development."}</p></>}</div><div className="goal-actions">{editing ? <><button type="button" className="landing-button" onClick={saveGoal} disabled={busy !== null}>Save changes</button><button type="button" className="button-quiet" onClick={() => setEditing(false)}>Cancel</button></> : <><button type="button" className="landing-button landing-button-secondary" onClick={() => setEditing(true)} aria-label="Edit goal">Edit goal</button>{goal.status === "completed" ? <button type="button" className="landing-button landing-button-secondary" onClick={() => setGoalStatus("active")} disabled={busy !== null}>Reopen</button> : <button type="button" className="landing-button landing-button-secondary" onClick={() => setGoalStatus("completed")} disabled={busy !== null}>Mark complete</button>}<button type="button" className="button-danger" onClick={() => setArchiveOpen(true)} disabled={busy !== null} aria-label="Archive goal">Archive</button></>}</div></section><p className="goal-progress-count" role="status">{completedCount} of {focusCount} focus areas marked complete</p>{error && <p className="form-error" role="alert">{error}</p>}<section className="goal-current-focus" aria-labelledby="current-focus-title"><div><p className="eyebrow">Current focus</p><h2 id="current-focus-title">{progress?.current_focus?.label ?? goal.focus_areas.find((item) => item.status === "active")?.title ?? "Review your goal"}</h2><p className="muted">{nextAction.reason}</p></div>{nextAction.action.kind === "continue_conversation" && nextAction.action.conversation_id ? <Link className="landing-button" href={`/conversations/${nextAction.action.conversation_id}`}>Continue practice</Link> : nextAction.action.kind === "start_practice" && nextAction.focus_area_id ? <button type="button" className="landing-button" onClick={async () => { setBusy("launch"); try { const conversation = await launchFocusAreaPractice(supabase, goal.id, nextAction.focus_area_id!); router.push(`/conversations/${conversation.id}`); } catch (cause) { setError(safeError(cause)); } finally { setBusy(null); } }} disabled={busy !== null}>Start practice</button> : <Link className="landing-button landing-button-secondary" href="#focus-areas">Review focus areas</Link>}</section><section id="focus-areas" className="goal-focus-list" aria-labelledby="focus-areas-title"><div className="summary-heading"><p className="eyebrow">Plan</p><h2 id="focus-areas-title">Focus areas</h2></div>{goal.focus_areas.filter((item) => item.status !== "archived").map((item, index) => <article className="goal-focus-row" key={item.id}><div className="goal-focus-row-main"><span className="status-pill">{item.status === "completed" ? "Completed" : modeLabels[item.practice_mode]}</span><h3>{item.title}</h3><p className="muted">{item.description}</p><p className="field-hint">{progress?.focus_areas.find((value) => value.focus_area_id === item.id)?.linked_practiced_sessions ?? 0} linked practice sessions · {progress?.focus_areas.find((value) => value.focus_area_id === item.id)?.linked_user_turns ?? 0} user turns</p></div><div className="goal-focus-row-actions"><button type="button" className="button-quiet" onClick={() => { setEditingFocus(item); setFocusTitle(item.title); }} disabled={busy !== null} aria-label={`Edit ${focusAreaAccessibleName(item.title, index)}`}>Edit</button><button type="button" className="button-quiet" onClick={() => updateFocus(item.id, { status: item.status === "completed" ? "active" : "completed" })} disabled={busy !== null} aria-label={`${item.status === "completed" ? "Reopen" : "Complete"} ${focusAreaAccessibleName(item.title, index)}`}>{item.status === "completed" ? "Reopen" : "Complete"}</button><button type="button" className="button-quiet" onClick={() => moveFocus(index, -1)} disabled={index === 0 || busy !== null} aria-label={`Move ${focusAreaAccessibleName(item.title, index)} up`}>Up</button><button type="button" className="button-quiet" onClick={() => moveFocus(index, 1)} disabled={index === goal.focus_areas.length - 1 || busy !== null} aria-label={`Move ${focusAreaAccessibleName(item.title, index)} down`}>Down</button><button type="button" className="button-danger" onClick={() => setArchivingFocus(item)} aria-label={`Archive ${focusAreaAccessibleName(item.title, index)}`}>Archive</button></div></article>)}</section><Dialog open={archiveOpen} title="Archive this goal?" description="Archived goals move to Goal history and become read-only. Their focus areas will no longer be available for practice." onClose={() => setArchiveOpen(false)}><div className="dialog-actions"><button type="button" className="button-danger" onClick={() => void archive()} disabled={busy !== null}>Archive goal</button><button type="button" className="button-secondary" onClick={() => setArchiveOpen(false)}>Cancel</button></div></Dialog><Dialog open={editingFocus !== null} title="Edit focus area" description="Update the focus area title used in your practice plan." onClose={() => setEditingFocus(null)}><form className="dialog-form" onSubmit={(event) => { event.preventDefault(); void editFocusTitle(); }}><label htmlFor="focus-area-title">Focus area title</label><input id="focus-area-title" value={focusTitle} maxLength={120} onChange={(event) => setFocusTitle(event.target.value)} /><div className="dialog-actions"><button type="submit" disabled={busy !== null}>Save title</button><button type="button" className="button-secondary" onClick={() => setEditingFocus(null)}>Cancel</button></div></form></Dialog><Dialog open={archivingFocus !== null} title="Archive focus area?" description="This removes the focus area from the active plan. It will remain visible in the goal history." onClose={() => setArchivingFocus(null)}><div className="dialog-actions"><button type="button" className="button-danger" onClick={() => { if (!archivingFocus) return; void archiveFocusArea(supabase, goal.id, archivingFocus.id).then(() => { onChanged({ ...goal, focus_areas: goal.focus_areas.map((value) => value.id === archivingFocus.id ? { ...value, status: "archived" } : value) }); setArchivingFocus(null); }).catch((cause) => setError(safeError(cause))); }}>Archive focus area</button><button type="button" className="button-secondary" onClick={() => setArchivingFocus(null)}>Cancel</button></div></Dialog></div>;
}
