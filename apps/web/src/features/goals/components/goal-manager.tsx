...
function ActiveGoal({ goal, initialProgress, onChanged, onArchived }: { goal: Goal; initialProgress: GoalProgress | null; onChanged: (goal: Goal) => void; onArchived: (goalId: string) => Promise<void> }) {
  ...
  const archive = async () => { if (!window.confirm("Archive this goal? Archived goals are read-only.")) return; setBusy("archive"); setError(null); try { await archiveGoal(supabase, goal.id); await onArchived(goal.id); } catch (cause) { setError(safeError(cause)); } finally { setBusy(null); } };
